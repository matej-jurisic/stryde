# Stryde release-APK builder. One command: bump version -> build SPA -> cap sync -> sign -> assemble
# -> stage for distribution. Run from the repo root:
#   .\stryde-build.ps1
#
...
$keystore = "D:\Projects\stryde-release.jks"        # kept OUTSIDE the repo
$alias    = "stryde"
...
$remoteUser = "matej"
$sshPort    = 22
$remoteDir  = "/data/stryde/releases"  # Obtainium URL -> .../releases/stryde.apk
...
$versioned = "$outDir\stryde-v$newName-$newCode.apk"
$stable    = "$outDir\stryde.apk"
...
if ($remoteHost) {
    ...
    scp -P $sshPort "$stable"    "${remoteUser}@${remoteHost}:$remoteDir/stryde.apk"
